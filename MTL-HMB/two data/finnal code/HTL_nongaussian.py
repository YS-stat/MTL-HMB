...
def run_simulation_HTL(p1, p2, p3, rho1, rho2, n_t, n_s, alpha, sigmat, sigmas, num_seeds, output_dir):
    ...
    for seed in tqdm(range(num_seeds), desc="Processing Seeds"):
        ...
        set_seed(seed)
        # Generate multivariate non-Gaussian features
        p = p1 + p2 + p3
        
        def generate_correlated_data(rho, n_samples, p, nonlinear=True):
            cov = torch.zeros(p, p)
            for i in range(p):
                for j in range(p):
            ...
            if nonlinear:
                # Apply nonlinear transformation to simulate non-Gaussian features
                X = X + 0.3 * torch.sin(X)
        
            return X
        
        # Simulate data for target and source domains
        X_t = generate_correlated_data(rho1, n_t, p)
        X_s = generate_correlated_data(rho2, n_s, p)
        
        # Split into three feature blocks
