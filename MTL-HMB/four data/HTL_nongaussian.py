...
class Integmodel(nn.Module):
    ...
    def forward(self, x_t, x_s, x_r, x_u):
        ...
        return y_t, y_s, y_r, y_u, orth, norm_products 
    ...
    for dim, depth, lr, lambda_orth, lambda_red in itertools.product(dims, depths, lrs, lambda_orths, lambda_reds):
        ...
        for epoch in range(num_epochs):
            ...
            for ((X_t, y_t), (X_s, y_s), (X_r, y_r), (X_u, y_u)) in zip(train_loader_t, train_loader_s, train_loader_r, train_loader_u):
                optimizer.zero_grad()
                y_t_pred, y_s_pred, y_r_pred, y_u_pred, orth, red = model(X_t, X_s, X_r, X_u)
                loss_t = criterion(y_t_pred, y_t)
                loss_s = criterion(y_s_pred, y_s)
                loss_r = criterion(y_r_pred, y_r)
                loss_u = criterion(y_u_pred, y_u)
                total_loss = loss_t + loss_s + loss_r + loss_u + lambda_orth * orth + lambda_red * red 
                total_loss.backward()
                optimizer.step()
            scheduler.step()  
            ...
            for ((X_t, y_t), (X_s, y_s), (X_r, y_r), (X_u, y_u)) in zip(val_loader_t, val_loader_s, val_loader_r, val_loader_u):
                with torch.no_grad():
                    y_t_pred, y_s_pred, y_r_pred, y_u_pred, _, _ = model(X_t, X_s, X_r, X_u)
                    loss_t = criterion(y_t_pred, y_t)
                    loss_s = criterion(y_s_pred, y_s)
                    loss_r = criterion(y_r_pred, y_r)
                    loss_u = criterion(y_u_pred, y_u)
...
for seed in range(num_seeds):  
    ...
    X_u_hat=torch.cat([X_u_1, X_u_5],dim=1)
...
filename = f"HTL_four_data_nongaussian_seeds={num_seeds}.csv"
