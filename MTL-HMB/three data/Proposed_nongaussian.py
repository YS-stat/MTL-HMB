...
class Integmodel(nn.Module):
    ...
    def forward(self, x_t, x_s, x_r):
        ...
        rob_penalty = 1*torch.sum(self.unique_encoder_t[0][0].weight[:, p1+p2:]**2) +1* torch.sum(self.unique_encoder_s[0][0].weight[:, p1:p1+p2]**2) + 1* torch.sum(self.unique_encoder_s[0][0].weight[:, p1+p2+p3:]**2) + 1* torch.sum(self.unique_encoder_r[0][0].weight[:, p1:p1+p2+p3]**2)
    

        return y_t, y_s, y_r, orth, rob_penalty, norm_products 
...
def grid_search_integmodel(X_t_hat, y_t, X_s_hat, y_s, X_r_hat, y_r, seed):
    ...
    num_epochs = 25000
    input_dim=p1
    input_dim_t=p1+p2+p3+p4
    input_dim_s=p1+p2+p3+p4
    input_dim_r=p1+p2+p3+p4
    ...
    for dim, depth, lr, lambda_orth, lambda_rob, lambda_red in itertools.product(dims, depths, lrs, lambda_orths, lambda_robs, lambda_reds):
        ...
        for epoch in range(num_epochs):
            ...
            for ((X_t, y_t), (X_s, y_s), (X_r, y_r)) in zip(train_loader_t, train_loader_s, train_loader_r):
                optimizer.zero_grad()
                y_t_pred, y_s_pred, y_r_pred, orth, rob, red = model(X_t, X_s, X_r)
                loss_t = criterion(y_t_pred, y_t)
                loss_s = criterion(y_s_pred, y_s)
                loss_r = criterion(y_r_pred, y_r)
                total_loss = loss_t + loss_s + loss_r + lambda_orth * orth + lambda_rob * rob + lambda_red * red 
                total_loss.backward()
                optimizer.step()
            scheduler.step()  
            ...
            for ((X_t, y_t), (X_s, y_s), (X_r, y_r)) in zip(val_loader_t, val_loader_s, val_loader_r):
                with torch.no_grad():
                    y_t_pred, y_s_pred, y_r_pred, _, _ , _= model(X_t, X_s, X_r)
                    loss_t = criterion(y_t_pred, y_t)
                    loss_s = criterion(y_s_pred, y_s)
                    loss_r = criterion(y_r_pred, y_r)
...
for seed in range(num_seeds):  
    ...
    X_sr_1 = torch.cat([X_s_1, X_r_1],dim=0)
    X_sr_1_train, X_sr_1_val = train_test_split(X_sr_1, test_size=0.1, random_state=42)
   
    X_sr_2_hat_best=imputation_grid_search(X_t_1_train, X_t_1_val, X_sr_1_train,                                           X_sr_1_val, X_t_2_train, X_t_2_val, X_sr_1, input_dim=p1, output_dim=p2, seed=seed)
    X_s_2_hat_best=X_sr_2_hat_best[:n_s,:]
    X_r_2_hat_best=X_sr_2_hat_best[n_s:,:]
    ...
    X_tr_1 = torch.cat([X_t_1, X_r_1],dim=0)
    X_tr_1_train, X_tr_1_val = train_test_split(X_tr_1, test_size=0.1, random_state=42)
   
    X_tr_3_hat_best=imputation_grid_search(X_s_1_train, X_s_1_val, X_tr_1_train,                                           X_tr_1_val, X_s_3_train, X_s_3_val, X_tr_1, input_dim=p1, output_dim=p3, seed=seed)
    X_t_3_hat_best=X_tr_3_hat_best[:n_t,:]
    X_r_3_hat_best=X_tr_3_hat_best[n_t:,:]
    ...
    X_ts_1 = torch.cat([X_t_1, X_s_1],dim=0)
    X_ts_1_train, X_ts_1_val = train_test_split(X_ts_1, test_size=0.1, random_state=42)
   
    X_ts_4_hat_best=imputation_grid_search(X_r_1_train, X_r_1_val, X_ts_1_train,                                           X_ts_1_val, X_r_4_train, X_r_4_val, X_ts_1, input_dim=p1, output_dim=p4, seed=seed)
    X_t_4_hat_best=X_ts_4_hat_best[:n_t,:]
    X_s_4_hat_best=X_ts_4_hat_best[n_t:,:]    
    
    X_t_hat=torch.cat([X_t_1, X_t_1, X_t_2, X_t_3_hat_best, X_t_4_hat_best],dim=1)
    X_s_hat=torch.cat([X_s_1, X_s_1, X_s_2_hat_best, X_s_3, X_s_4_hat_best],dim=1)
    X_r_hat=torch.cat([X_r_1, X_r_1, X_r_2_hat_best, X_r_3_hat_best, X_r_4],dim=1)
...
filename = f"new_proposed_three_data_nongaussian_seeds={num_seeds}.csv"
